...
inline void solve() {
    ...
    auto sol = [&](int l, int r, auto&& sol) -> int {
        if (l == r) return fp(a[l], k, mod);
        int mid = (l+r) >> 1;

        vector<int> sl(k + 1), sr(k + 1);
        int sum = 0;
        ...
        int res = (sol(l, mid, sol) + sol(mid+1, r, sol)) % mod;
        int tmp = 0;
        // merge
        for (int i = 0; i <= k; i++) {
            res = (res + sl[i] * sr[k - i] % mod * C(k, i) % mod) % mod;
            tmp = (tmp + sl[i] * sr[k - i] % mod * C(k, i) % mod) % mod;
            //test(i, sl[i], sr[i]);
        }
        //test(l, r, tmp);
        return res;
